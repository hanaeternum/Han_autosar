...
- 用按钮模拟应用层动作：**整车 KL15 上下电、主动唤醒 / 关闭唤醒源、释放全部需求、重复报文请求、诊断请求**，并能**手工注入自定义 NM 报文**；
...
产物在 `dist/`（单文件，各约 37 MB，目标机无需安装 Python）：

| 文件 | 用途 |
| --- | --- |
| `CanNmSim.exe` | 双击即用，无控制台黑框 |
| `CanNmSim-console.exe` | 带控制台，用于自检：`CanNmSim-console.exe --smoke` |

两个 exe 的命令行参数与 `main.py` 完全一致（见第 6 节）。打包时 `config/network.json`
已内嵌进 exe；**如果往 exe 旁边放一份同名 `config/network.json`，程序会优先读旁边这份**，
所以后续改配置不必重新打包（路径逻辑见 `src/config/paths.py`）。

两个提醒：

- 排查启动问题请用**控制台版**。GUI 版没有 stdout，配置写错时只会静默退出、看不到任何报错。
- 只要 GUI 版就够：`python tools/build_exe.py --only gui`；
  想让启动更快（代价是要连目录一起分发）：加 `--onedir`。

### 3.3 无界面场景回放（内核自检 / 出报告用）
...
| 关闭 IO / 关闭 FUNC | 与"主动唤醒"成对：`CanNm_NetworkRelease(source)`，**只清除下拉选中那一路**保持事件，KL15 / 诊断保持不受影响。按钮文字跟着下拉走，且仅当该源确实持有需求时可点 |
| 当前网络需求 | 实时显示本机节点还挂着哪些保持事件（如 `KL15`，琥珀色）或"无"。**只有存在需求时「释放全部需求」按钮才可点** |
| 释放全部需求 | `CanNm_NetworkRelease()`，一次清除本节点全部保持事件。**注意：在 RMS 里点它不会立刻跳走**，要先等 T_REPEAT_MESSAGE 超时（规范 4.3.3.1） |
| 重复报文(RMR) | `CanNm_RepeatMessageRequest()`。把节点拉回 RMS、RMR 位在报文中置 1、并启用快速发送 |
| 诊断请求 | 在 RSS 下会把节点拉回 NOS 并启动 T_WAIT_DiagReq（规范 4.3.3.3） |
| 自定义报文… | 打开对话框手工构造一帧 NM 报文注入总线 |
...
src/
  config/                      配置加载与校验
    paths.py                     资源路径解析（源码运行 / 打包成 exe 双模式）
...
tools/
  build_exe.py                 打包成 Windows exe（PyInstaller）
...
| 打包成 exe / 改打包参数 | `tools/build_exe.py`（产物在 `dist/`，已在 .gitignore 里） |
...
**Q4：诊断报文能不能从 BSM 唤醒网络？**
**不能，规范里就是这么写的。** 庆铃规范表 4（各运行模式下可收发的报文类型）中，
`BusSleepMode` 与 `PrepareBusSleepMode` 的**「应用报文 Rx」都是 N** —— 这两个状态下节点
根本不接收应用报文（诊断报文属于应用报文），所以诊断报文既不能从 BSM 唤醒网络，也不能打断 PBS 的休眠流程。

诊断只在网络模式内起作用，且只在 RSS 上能改变状态（规范 4.3.3.3）：

| 当前状态 | 收到诊断报文 | 结果 |
| --- | --- | --- |
| BSM | 不接收 | 保持 BSM（界面上「模拟诊断请求」按钮会置灰） |
| PBS | 不接收 | 保持 PBS（按钮同样置灰） |
| RMS | 接收 | 保持 RMS，按常规流程走 |
| NOS | 接收 | 重启 `T_WAIT_DiagReq` 继续保持 |
| **RSS** | 接收 | **进 NOS** 并启动 `T_WAIT_DiagReq`，5 秒无新诊断报文后回 RSS |

真实车辆里诊断仪确实能"唤醒"整车，但走的不是这条路径：诊断仪的报文先唤醒 CAN 收发器 →
ECU 上电 → 应用层调用 `Nm_NetworkRequest()`。在界面上演示这一串的等价操作是**先点一次
「KL15 上电」或「主动唤醒」**，等节点进入网络模式后再点「模拟诊断请求」。

如果客户要求"诊断报文即可直接唤醒网络"，把 `config/network.json` 里的
`protocol.diag_can_wake_from_sleep` 改成 `true` 即可（行为与 RSS 一致：直接进 NOS 并启动
`T_WAIT_DiagReq`）。注意这会绕过"进入网络模式默认先进 RMS"的规则，所以默认关闭，
并已列入待确认清单第 7 条。

**Q5：点了「KL15 上电」后节点一直停在 NOS（绿色），是不是卡住了？**
...
**Q11：报文表里出现源节点为 `EXT` 的「注入」报文是什么？**
是你通过「发送自定义 NM 报文」手工注入的帧，用来模拟外部测试节点或复现抓包。

**Q12：界面看起来正常但一个字都没有（截图场景）**
...
| 6 | **诊断报文能否在 BSM / PBS 下唤醒网络**：规范表 4 中这两个状态的「应用报文 Rx」为 N，当前实现为**不能** | 默认关闭该行为，配置项 `diag_can_wake_from_sleep` 可开启（开启后绕过"默认先进 RMS"） |
| 7 | **AC_4_OTA 的位序**：规范表格里写作 AC_3_OTA，疑为笔误 | 按 Byte4 bit4 处理 |
